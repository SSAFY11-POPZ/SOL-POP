...
const ReservationDrawer = ({ onClose, storeId }) => {
    ...
    try {
      ...
      if (data.reserveDate) {
        ...
        if (Array.isArray(data.unavailableTime)) {
          setUnavailableTimes(data.unavailableTime.map(time => time.slice(0, 5))); // Slicing to "HH:mm" format
        } else {
  ...
  const handleReservationSubmit = async () => {
    ...
    try {
      const response = await axios.post(`https://solpop.xyz/api/v1/store/${storeId}/reserve/request?datetime=${datetime}`, {}, {
        headers: {
        ...
        <div className="flex justify-center mb-4">
          <Calendar
            ...
            tileClassName={({ date, view }) => {
              let classes = 'p-1.5 text-center text-gray-900 w-8 h-8 flex items-center justify-center';
            ...
            navigationLabel={({ date, view }) => <span className="text-lg font-bold">{`${date.toLocaleString('default', { month: 'long' })} ${date.getFullYear()}`}</span>}
            locale="en-US"
          />
        </div>
